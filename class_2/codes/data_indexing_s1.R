...
#   use double [[]]
# e.g. df[[1]] will give you the first column in vector
df[[1]]
# however df[[,1]] will not work...
df[[,1]]
# specfy the row element as well: first element from first column
df[[1,1]]
# getting multiple elements (first 3 elements of first column)
df[1:3,1]


#CALLING VARIABLE BY ITS NAME WITH $
...
# Lets find age of 3rd observation of the age variable:
df$age[ 3 ]
# Or one can use logical expressions as well to get the same variable:
df$age[df$id==3]
...
df$grade[ df$age >= 25 & df$age < 35 ]  #note that the missing value is included by default
...
df$id[ which( df$grade == "A" | df$grade == "A+" ) ]# Extra: use the `which()` function to find these values instead
...
logID <- df$grade == "B+" | df$grade == "B" | df$grade == "B-" 
logID <- df$grade != "A"  | df$grade != "A+"
...
rbind( df , df_fj[ , 1 : 3 ] )  #take original df and merge with new df but without the gender columns
...
df_new3 <- tibble(df_new2, year = 2002)
df_new3
