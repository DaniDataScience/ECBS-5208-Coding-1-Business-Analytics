...
library(readr)
data_in <- ("C:/Users/Dani/CEU/Data_mgnt_and_analyis_with_R/ECBS-5208-Coding-1-Business-Analytics/class_2/data/hotels/")
...
data_in <- ("C:/Users/Dani/CEU/Data_mgnt_and_analyis_with_R/ECBS-5208-Coding-1-Business-Analytics/class_2/data/hotels/")
df_0 <- read_csv(paste0(data_in,"raw/hotelbookingdata.csv"))
View(df_0)

# 4) also load the data directly from the web
df <- read_csv(url("https://osf.io/yzntm/download"))
df

# 5) write out this file as xlsx and as a .RData next to the original data.
data_out <- paste0( data_in , 'clean/' )
write_csv( df , paste0( data_out , 'hotelseurope.csv' ) )
